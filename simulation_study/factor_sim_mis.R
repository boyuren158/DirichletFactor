# Simulation study when prior is not the data generating process
...
sfLapply( seq( opt$start.idx, opt$end.idx ), function(x){
  sim.res.shrink.test = sim.CDP.shrink.test( c(1000,10000,100000), 22, 68, opt$tru.factor, opt$tru.block, a = 1, 3, 1, sigma.value, sigma.prior )
  ...
  for( id in 1:length( sim.res.shrink.test$data ) ){
	...
	start = list( sigma = sample( sigma.value, size = length( sim.res.shrink.test$sigma ), replace = T, prob = sigma.prior ), 
                ...
                X = matrix( rnorm( 10*68 ), nrow = 10 ),
                Y = matrix( rnorm( 10*22 ), nrow = 10 ),
                er = 1/rgamma( 1, shape = hyper$a.er, rate = hyper$b.er ),
                delta = c( rgamma( 1, shape = 3, rate = 1 ), rgamma( 9, shape = 4, rate = 1 ) ),
                phi = matrix( rgamma( 10*22, shape = 3/2, rate = 3/2 ), nrow = 22 ) )
